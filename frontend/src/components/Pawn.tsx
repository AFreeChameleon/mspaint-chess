...
class Pawn extends React.Component<PawnProps, PawnState> {
    ...
    render() {
        ...
        const isDragging = chess.isDragging && chess.selectedSquare === square;
        const draggable = chess.currentTurn === colourName && chess.game.moves({square}).length > 0 && chess.game.moves({square}).length > 0;
        
        return (
            <div 
                draggable="false"
                id={`pawn_${colourName}_${square}`}
                style={isDragging ? { 
                    backgroundImage: `url(/img/pawn_${colourName}.png)`, 
                    position: 'absolute',
                    top: position.y + 'px', 
                    left: position.x + 'px',
                    pointerEvents: 'none'
